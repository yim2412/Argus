...
## ▶ 재개 지점 (2026-09-25 16:45 — **분석 끝, 사용자 합의 대기**)
...
#### 판단 필요 (수정 전에 사용자가 정한다)

| 항목 | 물음 | 선택지 |
|---|---|---|
| F-001 | `rules.yaml` 머리말의 "사용자 룰은 `%APPDATA%\Argus\rules.yaml`" 약속을 | (a) 지킨다 — 사용자 파일을 읽는다(F-003·004·021 이 따라온다) (b) 지운다 — 머리말만 고친다 |
| F-026 | 첫 실행에 기본값 **전체**를 복사하는 사본을 | (a) 주석만 있는 빈 템플릿으로 (b) 사본에 버전 + 옮겨 쓰기 (c) 둘 다. 이미 퍼진 사본 처리도 |
| F-013 | 새 버전 DB 를 옛 exe 로 열 때 | 막는다 / 경고만 한다 |
| F-025 | 스모크 블록 51줄(`python -m …` 자체 점검)을 | pytest 로 감싸 CI 에 넣는다 / 지금처럼 손으로 돌린다 |
