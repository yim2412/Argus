...
**룰을 바꾸려면** `%APPDATA%\Argus\rules.yaml` 을 만듭니다(동봉 `argus/config/rules.yaml` 을 복사해 고치는
것이 쉽습니다). 있으면 동봉 룰을 **통째로** 대체합니다. 파일이 틀렸으면(문법·구조·없는 지표 이름) 기본 룰로
계속 돌고 창 맨 위에 "설정 확인이 필요합니다 — 이유"가 뜹니다. 고친 뒤 상주를 다시 시작하면 적용됩니다.
