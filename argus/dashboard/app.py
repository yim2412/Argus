...
지금 있는 페이지는 데이터가 이미 존재하는 것들뿐이다 — Regimes(Phase 4-B),
Incidents(Phase 9), Models(학습된 모델)는 그 단계가 오면 붙인다. 빈 화면을
...
- **Live** — 지금 이 순간의 상태와 최근 10분
- **Timeline** — 1분 집계 시간축. 결함 주입 구간과 탐지 신호가 겹쳐 보인다
- **Processes** — 프로세스별 사용량 랭킹과 포어그라운드
- **Health** — Argus 자신의 상태. 관측자가 병목이 되고 있지 않은지

준비 중: Regimes(Phase 4-B) · Incidents(Phase 9) · Models(학습 후)
"""
)
