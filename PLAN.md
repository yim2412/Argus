...
**DoD**: 결함 주입 시나리오 6개 중 4개 이상을 룰만으로 탐지, 정상 구간 오탐 < 1/h.

> ⚠️ **거의 완료 — 시나리오 항목만 남았다.**
>
> | 항목 | 상태 |
> |---|---|
> | 기준선 초과 | ✅ `rules` F1 **0.615** > `fixed_cpu` 0.364 > `always` 0.214 |
> | 오탐 0건 | ✅ FP 0 / 정밀도 100% |
> | 오탐률 < 1/h | ✅ **0.00/h** (`always` 는 60.83/h — 비교군이 제 역할을 했다) |
> | 시나리오 4개 이상 | ❌ **2종**(`cpu_spin`·`memory_leak`)만 |
>
> **DoD 문구 수정 제안**: "6개 중 4개"는 주입 시나리오가 6개라는 전제로 쓴 것인데,
> 실제 주입기는 4종이고 그중 2종은 **이 하드웨어에서 증상 자체가 발생하지 않는다.**
> 증상 없는 구간을 탐지하라는 요구는 오탐을 요구하는 것과 같다(CLAUDE.md: 증상 없는
> 원인은 알릴 가치가 없다). 기준을 **"증상이 관측되는 시나리오는 전부 탐지"** 로 바꾸면
> 현재 2/2 로 충족이다. GPU 점유 시나리오를 추가하면 3종으로 늘릴 수 있다.
>
