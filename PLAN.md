...
> ⚠️ **`process.parquet` 은 아직 없다 (2026-07-27).** 4-A 에서 `metrics` 만 만들었다.
> 그 결과 프로세스 데이터는 여전히 24시간 뒤 사라지고, Phase 6(지문)이 학습할 것이
> 남지 않는다. 실행 순서 1번이 이것이다(§8).
>
...
> ⚠️ **선행 조건: 프로세스 롤업 (2026-07-27 발견).**
>
> 이 Phase 는 "프로세스명 × 레짐 단위 p50/p95/p99" 를 학습하는데, **학습할 데이터가
> 매일 사라진다.** Phase 4-A 는 `metrics_raw` 의 증발만 막았고 `process_metrics` 는
> 24시간 보존 그대로다. `metrics_1m` 에 남는 프로세스 정보는 `foreground_proc`,
> `top_cpu_proc`, `top_cpu_share` 셋뿐이라 분포를 복원할 수 없다.
>
> 더 나쁜 것은 **보호 장치가 헛돌고 있다는 점**이다. `process_metrics` 는 보존 규칙에서
> 롤업 워터마크에 묶여 있는데(`needs_rollup=True`), 정작 그 롤업은 프로세스 데이터를
> 접지 않는다. 접히지도 않은 채 "롤업이 지나갔으니 안전하다"는 이유로 지워진다.
>
> `PLAN.md` 3.2 에 `process.parquet (5분 집계, 상위 프로세스만)` 이 이미 적혀 있었으나
> 4-A 에서 빠뜨렸다. **Phase 6 착수 전에 채운다.**
...
| # | 작업 | 근거 | 여는 것 |
|---|---|---|---|
| 1 | **프로세스 롤업** | **지연 비용 있음** — `process_metrics` 보존 24시간, 이후 영구 손실. 게다가 보호 장치가 헛돌고 있다(워터마크에 묶여 있으나 접히지 않는다) | Phase 6, 13 |
