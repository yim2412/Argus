...
| 3 | **Phase 6** 프로세스 지문 | 1 이 며칠 쌓인 뒤 | `handle_leak` 미탐, Phase 8 지문 가중, Phase 13 |
| 4 | **Phase 4-B** 레짐 추론 | 데이터 축적 필요 | 레짐 조건부 베이스라인, 대시보드 Regimes |
| 5 | **Phase 11** 피드백 | 2 이후, 피드백이 모인 뒤 | 지속 학습 |
| 6 | **Phase 13** 보안 라이트 | ⏸️ **보류.** 데이터 의존이 없다고 봤으나 틀렸다 — 판정 기준을 세울 근거가 없다(위 표). 데이터 계층(`net_activity_5m`)만 먼저 넣었다 | — |
| 7 | **Phase 14** 패키징 | 마지막. 범위가 7 의 채택 여부에 달렸다. **자동 시작만 2026-07-28 에 앞당겼다** — 축적 대기 작업이 쌓여 있는데 상주 실행이 수동이면 그 대기가 늘어난다(원칙 1) | 배포 |
