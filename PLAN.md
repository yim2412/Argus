...
### 다음 세션 착수점 (2026-07-29 23:5x 기준)

**먼저 확인할 것 하나.** 지금 도는 상주 인스턴스(23:03 기동)는 **Phase 6-B 코드가 없다.**
지문 커밋이 그 뒤라 `FingerprintBuilder` 가 돌지 않고 `procleak` 도 억제 없이 판정한다.
지문 121건은 DB 에 이미 있으나 읽어 가는 쪽이 옛 코드다. PC 를 다시 켰다면 저절로
해소됐을 테니, `argus.jsonl` 에서 최근 기동 로그의 컴포넌트 목록에 `fingerprint` 가
있는지만 보면 된다. 없으면 `python -m argus --stop` 후 `schtasks /run /tn "Argus"`.

착수 가능한 것을 지연 비용 순으로:

| 순위 | 할 일 | 왜 지금 | 막는 것 |
|---|---|---|---|
| 1 | **Phase 8 재개봉** — 귀인의 신규 프로세스 과대 계상 | 데이터 대기가 없다. 유일하게 지금 바로 되는 코드 작업이고, 귀인은 "설명이 산출물"의 핵심이라 틀린 채로 두면 사건 리포트가 전부 오염된다 | 없음 |
| 2 | 알람 품질 ②③ | warning 표본은 **11건으로 충족됐다**(2026-07-29 23:5x 실측). 남은 건 **다양성 하나** — 관측 3일이 전부 고부하다 | 유휴 위주의 날 하루 |
| 3 | Phase 4-B 레짐 | 관측 3일/7일 | 나흘 |

**1번(Phase 8)의 구체적 착수 지점**: `argus/eval/attribution.py:112` 의 `before` 창은
`ts_start-150s ~ ts_start-30s` 인데, 이 창에 없던 프로세스가 구간 중에 뜨면 그 프로세스의
**초기 보유량 전체가 증가분**이 된다. 실측에서 `compattelrunner` 13개가 각자 400 핸들을
들고 떠 5,200 이 증가분으로 잡혔고, 혼자 +8,313 을 낸 실제 원인을 40% 대 23% 로 밀어냈다.
재현: `.venv\Scripts\python.exe -m argus.eval --attribution --scenario handle_leak`
(현재 1순위 지목률 50%, DoD 85%). 고칠 때 정할 것은 아래 "Phase 8 재개봉" 절에 있다.
...
#### Phase 8 재개봉 — 귀인이 새로 뜬 프로세스를 과대 계상한다 (2026-07-29)
