...
> ⚠️ **DoD 는 아직 닫히지 않았다 (2026-07-29).** 아래처럼 6-A / 6-B 로 쪼갰고 6-A 만 끝났다.
> DoD 의 "1위 용의자 지목"은 6-A 가 충족한다(주입 프로세스를 PID 로 직접 지목, score 1.00).
> **"정상 프로필 구축"은 6-B 몫이라 미착수다.** 오늘 닫은 것은 `handle_leak` 미탐이라는
> 구체적 구멍이지 Phase 6 전체가 아니다.
...
| **6-B** | 지문 테이블 + p99 이탈 — **6-A 를 스코어보드에서 이겨야 채택** | 미착수 |
...
**6-B 가 실제로 개선할 지점** — 6-A 의 FP 2건은 둘 다 `medal`(게임 녹화)이었다.
...
|---|---|---|---|
| 0 · 1 · 2 · 3 · 4-A · 9 · 10 | **6**(6-A 만 완료) · **8**(재개봉) | 4-B · 6-B · 11 | 5 · 7 · 13 |

> **2026-07-29 밤 갱신.** 표에서 둘이 옮겨갔다. **Phase 6** 은 6-A 만 끝나 DoD 미달이고
> (정상 프로필 구축이 6-B 몫), **Phase 8** 은 오늘 채점에서 원래 있던 결함이 드러나
> 완료에서 내렸다. "착수했는데 못 끝낸 Phase 는 이제 없다"던 앞 문단은 더 이상 맞지 않다.
...
| 4b | **Phase 6-B** 프로세스 지문 | 6-A 를 스코어보드에서 이겨야 채택 | Phase 8 지문 가중, Phase 13 |
